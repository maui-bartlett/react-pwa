...
import { useFabUTokens } from './AccountThemeContext';
...
function CampaignInviteSection({ campaignId }: { campaignId: Id<'campaigns'> }) {
  ...
  const [inviteCreateFailed, setInviteCreateFailed] = useState(false);
  ...
  useEffect(() => {
    if (
      !inviteState ||
      !inviteState.canManage ||
      inviteState.link !== null ||
      busy ||
      inviteCreateFailed
    ) {
      return;
    ...
    void createInviteLink({ campaignId })
      .then(() => setInviteCreateFailed(false))
      .catch(() => setInviteCreateFailed(true))
      .finally(() => setBusy(false));
  }, [inviteState, busy, inviteCreateFailed, createInviteLink, campaignId]);
  ...
  async function regenerate() {
    setBusy(true);
    setCopied(false);
    setInviteCreateFailed(false);
    try {
      await regenerateInviteLink({ campaignId });
    } catch {
      setInviteCreateFailed(true);
    } finally {
      setBusy(false);
    }
  }
...
function AccountMenu({
  ...
  async function addCharacter() {
    ...
    if (gameSystem !== 'fabula-ultima') {
      setError('Character creation is not configured for this game system.');
      return;
    }
  ...
  return (
    <>
      ...
      >
        ...
        <DialogContent sx={{ p: 1.6, pt: 0.8, overflowX: 'hidden', overflowY: 'auto' }}>
          <Box
            key={screen}
            sx={{
