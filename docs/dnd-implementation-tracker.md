...
- [x] Add class leveling/multiclass management that derives hit dice, saves, proficiency options, spellcasting, and features from Convex.

  - Class/subclass edits now derive total level, hit dice pools, saving throw proficiencies, catalog proficiencies, spellcasting ability, and class summary features from the Convex class catalog.
  - Existing spent hit dice are preserved when class levels are edited.
...
- [x] Add campaign/GM support for DnD sheets if we want parity with FabU campaign tools.

  - DnD uses the shared account campaign system for campaign membership, invites, shared sheet visibility, and GM objective clocks.
  - The GM dashboard now renders DnD-specific sheet summaries instead of falling through to the FabU summary.

- [x] Add tests for DnD persistence, Convex sync, class catalog reads, and key UI flows.
  - DnD persistence/normalization unit tests cover Convex payload serialization/deserialization.
  - Class catalog derivation tests cover catalog-driven hit dice, saving throws, proficiencies, spellcasting, and class feature summaries.
  - A DnD Playwright smoke test covers the app shell and key management tabs.
